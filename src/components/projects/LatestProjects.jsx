...
export default class LatestCommits extends Component {
  ...
  async componentDidMount() {
    const response = await (
      await fetch("https://api.github.com/users/jokurale/repos")
    ).json();
    ...
    if (response.message) {
      ...
      this.setState(() => {
        return {
          isLoading: false,
          data: response.filter((repo) => repo.name !== "Jokurale"),
        };
      });
    }
  }
    ...
    if (this.state.isLoading) {
      return (
        <div className="LatestProjects">
          <div className="LatestProjects__Loader">
            <Loader size={10} color={"#302533"} loading />
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="LatestProjects">
          {this.state.data.map((repo, i) => {
            return <SingleRepo key={i} repo={repo} />;
          })}
        </div>
      );
    }
